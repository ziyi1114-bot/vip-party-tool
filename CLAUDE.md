...
`service-worker.js`：install 時 precache `PRECACHE_URLS`（HTML/CSS/JS/JSON/icons）。fetch 策略為**混合式**：`data/*.json` 走 **network-first**（先連網抓最新並更新快取、離線才回舊快取，讓「改完 JSON 重整即生效」成立），其餘靜態檔與圖片走 **cache-first**（圖片第一次連網後即離線可用）。**改動被 cache-first 快取的檔案（HTML/CSS/JS/圖片）後，若要強制使用者端重建快取，把 `CACHE_VERSION`（目前 `vpt-v2`）改成新值**，activate 會清掉舊版快取。
