...
- **改了 JS/CSS/HTML 一定要把 `service-worker.js` 的 `CACHE_VERSION` 加一號**（`vpt-v3`→`vpt-v4`…），否則已安裝的裝置會被 cache-first 卡在舊版。改 `data/*.json` 不用（network-first）。
- **「推上去卻沒更新」先看 Actions 分頁**：Pages 的「pages build and deployment」有 `build` 與 `deploy` 兩段。曾發生 **build 成功、deploy 失敗**（GitHub 偶發的暫時性失敗），此時 Pages 保留上一次成功版本、線上不會變。處理：到 Actions 對該筆按 **Re-run failed jobs**，或推一個（可空的）commit 重新觸發即可。用 `curl` 抓 `https://.../service-worker.js` 看 `CACHE_VERSION` 可快速判斷線上實際版本；用 `raw.githubusercontent.com/.../main/...` 則是看 repo（繞過 Pages CDN）。
